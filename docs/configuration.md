...
| `WSD_RETRIEVE_IMAGE_MAX_RETRIES` | `1` | Bounded automatic **RetrieveImage** retries after payload integrity failure or transport error (initial attempt plus this many retries). Set `0` for a single attempt. **Why:** truncated MTOM or transient TCP drops sometimes succeed on replay without operator intervention. |
